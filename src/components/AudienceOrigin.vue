...
export default {
  ...
  mounted() {
    ...
    data.forEach((item, index) => {
      ...
      if (index <= 4) {
        title.left = `${((215 * index) / 1120) * 100}%`;
        title.top = 40;
        if (index === 0) {
          title.left = '7%';
          obj.center = [`${(150 / 1000) * 100}%`, 60];
        } else {
          obj.center = [`${(((120 + 84) * index + 150) / 1120) * 100}%`, 60];
        }
      } else if (index === 5) {
        obj.center = [`${(150 / 1000) * 100}%`, 190];

        title.top = 180;

        title.left = '7%';
      } else {
        title.left = `${((215 * (index - 5)) / 1120) * 100}%`;
        title.top = 180;
        obj.center = [
          `${(((120 + 84) * (index - 5) + 150) / 1120) * 100}%`,
          190,
        ];
      }
